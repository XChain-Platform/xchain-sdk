...
### Fixed
- `src/checkpoint.js`, `src/stake_weighted_quorum.js` (new) — the light-client checkpoint verifier now matches the federation's quorum rule when stake-weighting is active. `verifyCheckpoint` previously applied only the count-based `2f+1` quorum over a bare-pubkey set, so on any network where stake-weighting is in effect it diverged from the signed-and-anchored consensus two ways: it **false-rejected** a legitimate checkpoint signed by a stake-heavy minority (one source holding >2/3 of stake but below the signature count), and it could **unsafe-accept** a checkpoint signed by a key-count majority that did not hold a stake majority. `verifyCheckpoint` now derives whether stake-weighting is active locally from the checkpoint's `snapshot_block` + `network` (the same flag-day gating the canonical signing string already uses for the equivocation header — the verdict never trusts a server-supplied flag), and when active applies the source-deduped `3·Σ > 2·S` predicate over the per-validator `{ pubkey, weight, source }` set, counting each stake source once no matter how many of its keys signed. Below the flag-day the existing count path is byte-for-byte unchanged. When weighting is active but the supplied set carries no weight/source (e.g. a legacy bare-pubkey list, or an explorer that hasn't been upgraded to return the enriched set) the verifier fails closed rather than silently counting. The result object gains a `weighted` boolean reporting which path decided. `stake_weighted_quorum.js` is a consensus-critical local copy of the hub/indexer predicate + activation map (mirroring the `equivocation_header.js` precedent), self-contained on exact `mathjs` bignumber math (`.gt`, not the epsilon-applying `mathjs.larger`) so large near-equal stake totals never flip the verdict at the boundary. New regression coverage in `test/unit/checkpoint.test.js` for the weighted-pass (stake-heavy minority), weighted-fail (key-count majority lacking stake), fail-closed (no weight/source), and below-flag-day count cases.
- `src/networks.js` — corrected the Litecoin `dustThreshold` from `546` to `5460` litoshis for `litecoin-mainnet` / `litecoin-testnet` / `litecoin-regtest`. Litecoin Core's dust relay fee is 10× Bitcoin's, so the effective dust floor for a standard output is 5460 litoshis; consumers building LTC transactions from these network objects previously risked emitting sub-dust fee/change outputs that the node rejects with `{"code":-26,"message":"dust"}`. Bitcoin and Dogecoin values are unchanged.
- `src/messaging.js` — encrypted `messaging.send()` (methods 1/2/3 — ECIES/ECDH/AES) no longer throws `NO_MATCHING_FORMAT`. `send()` set both `encryptedMessage` and `encryptionMethod` on the action params, but the MESSAGE v2 wire format (`VERSION|COIN|DESTINATION|ENCRYPTED_MESSAGE`) has no `ENCRYPTION_METHOD` slot, so the format selector's no-data-loss rule rejected every version and every encrypted send failed before a PSBT was built — taking the gated-content key-handoff unlock flow down end to end. MESSAGE v2 carries no method on the wire (absence implies ECIES per protocol), so `send()` no longer places `encryptionMethod` on the action params and the selector resolves v2. On the read side, `getMessages()` now infers ECIES when an encrypted body is present without a method, so method-less v2 rows — including legacy rows indexed before the decoder stamped a concrete method — decrypt and expose `msg.bytes` to the gated-content reader. New regression in `test/unit/messaging.test.js` drives `send()` through the **real** `createAction`/format selector (a `createAction` stub previously hid the throw) and adds a method-less v2 decrypt case.
