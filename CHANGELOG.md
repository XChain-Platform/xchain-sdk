...
- `src/explorer.js`, `src/XChainSDK.js`, `index.d.ts` — documented the two new `getStatus()` response fields, `node_tip` and `lag_blocks` (per-coin maps). `node_tip` is the chain tip (the explorer's view of it, from the decoder's highest seen block) and `lag_blocks` is `node_tip - last_block` (>= 0), so a consumer can detect a stalled indexer from a single `getStatus()` call rather than a separate out-of-band tip query. Both are `null` for a coin when the tip is unavailable. The SDK wrappers are thin pass-throughs of the explorer `/status` body, so the fields already flow through unchanged — this is a documentation-only change (method comments + the `index.d.ts` JSDoc, which previously told callers to "compare these against the chain tip" without surfacing the tip the API now provides).
