...
### Added
- The deploy workflow resolves a chunked contract through the explorer's reported contract index, so a group completed by a later carrier still resolves.

## [0.15.1] - 2026-09-07

### Added
- `sdk.freshness()` and `sdk.assertFresh()` read the explorer's tip-freshness marker, and `submitAction({ strictFreshness })` refuses to build on a stale tip.
