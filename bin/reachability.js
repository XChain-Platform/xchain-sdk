 ...
 *
 * AND IN A LIBRARY REPO THE STAKES ARE HIGHER STILL, because package.json's
 * `files` publishes the whole of src/: a consumer can deep-import any module in
 * it by its published path, which no sweep of this tree can see at all. A file
 * nothing here reaches is dead to this repo, never automatically dead to npm.
 *
 * THE FOUR REACHES, kept apart because they carry different weight:
 *
 *   runtime   the require closure of what consumers actually load: package.json
 *             `main`, the Dockerfile CMD, and every `node <file>` an npm script
 *             runs. A file outside this closure cannot execute in production.
 ...
 */
const DYNAMIC_EDGES = [
    // EMPTY, AND MEASURED RATHER THAN ASSUMED. Nothing under this repo's src/
    // builds a require path at runtime: a `command grep` for `require('./' +`,
    // require with a template literal, `require(path.join(` and `require(<var>)`
    // over src/ returns only prose inside comments. Every computed require in
    // this checkout sits in test/ or bin/ and names a SIBLING repo's file
    // (the indexer's actions, the explorer's ws modules, the documentation
    // repo's protocol constants), which the sibling half of the sweep covers.
    // A new computed edge inside src/ therefore shows up as a file that
    // suddenly reads unreachable, which is the failure mode this list exists
    // to make visible.
];
...
function runtimeEntries(fileSet) {
    ...
    // THE PACKAGE ENTRY POINT, which a service repo does not have and a library
    // repo lives by. `main` is what `require('@dankest-llc/xchain-sdk')` loads in
    // every consumer, so the whole published surface hangs off it; without this
    // edge the entire src/ tree reads unreachable from runtime and the candidate
    // list becomes every file in the repo.
    const pkgPath = path.join(REPO_ROOT, 'package.json');
    const pkgMain = JSON.parse(fs.readFileSync(pkgPath, 'utf8')).main;
    if (pkgMain) {
        const rel = pkgMain.replace(/^\.\//, '');
...
function twinCopies(sources, siblingsRoot) {
    ...
    try {
        repos = fs.readdirSync(siblingsRoot, { withFileTypes: true })
            .filter((e) => e.name.startsWith('xchain-') && e.name !== 'xchain-sdk')
            .map((e) => e.name)
            .sort();
    } catch (e) {
        return out;
...
function main() {
    ...
    console.log(`reachable from package runtime:      ${s.reachableFromRuntime}`);
    console.log(`NOT reachable from package runtime:  ${s.notReachableFromRuntime}`);
