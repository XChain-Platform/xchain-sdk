 ...
 *              `const R = path.resolve(__dirname, '../../xchain-sdk')` then
 *              path.join(R, 'src', 'foo.js'), path.join(R, 'src/foo.js') or
 *              `${R}/src/foo.js`. The root may equally be
 *              process.env.XCHAIN_SDK_PATH, a candidate array filtered to its
 ...
 *              this checkout, `sdkFile('src/rollback.js')`. A call over a
 *              literal array, `sdkFile('src/' + twin)`, is emitted once per
 ...
 *              `copy_twin xchain-sdk "src/$f"`, resolved against the literal
...
// `xchain-sdk/src/<path>`, however it was spelled: a relative require
// (`../../xchain-sdk/src/utility.js`), a prose mention, a shell path.
const TEXT_REFERENCE = new RegExp(`${NAME_RE}\\/(src\\/[A-Za-z0-9_@.\\-/]+)`, 'g');

// path.join(..., 'xchain-sdk', 'src', ...): the tail is captured raw and
...
// Environment variables that hold a checkout of this repo. XCHAIN_SDK_SQL_PATH
...
const ENV_ROOT_VARS = ['XCHAIN_SDK_PATH', 'XCHAIN_SDK_DIR'];
...
function rootPrefix(init) {
    ...
    while ((m = re.exec(init)) !== null) {
        ...
        // path.join(base, 'xchain-sdk', 'src', 'sql').
...
function collectRootProducers(text) {
    ...
    while ((m = re.exec(text)) !== null) {
        ...
        // `const p = sdkFile('src/x.js')` look like another checkout.
 ...
 * checkout, the `sdkFile('src/rollback.js')` idiom. Recognised by a body
...
function helperReferences(text, helpers, lists, strings) {
    ...
    for (const [name, suffix] of helpers) {
        ...
        let m;
        while ((m = re.exec(text)) !== null) {
            // `function sdkFile(rel){` reads as a call of itself. Its
 ...
 * relative path, `copy_twin xchain-sdk "src/$f"`, where the file comes from
