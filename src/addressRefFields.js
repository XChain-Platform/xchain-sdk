...
//
// The gate is built per action because deriving it from the FLAT SDK_COMPACTABLE
// name set minus each action's `noCompact` fields silently defeats `multi`. SEND
// declares {field:'DESTINATION', multi:true} and so is kept out of the flat
// set by its own spec. But MINT/MESSAGE/SWEEP each declare a single-value
// DESTINATION, so the NAME enters the flat set and comes straight back to SEND.
// A name-based gate therefore compacts single-recipient SEND destinations, and
// when `src/actions/send.js` was the ONE address-bearing handler with no
// resolveAddressRef call (every other action resolved), those sends were
...
// assigns it, the first send to any address succeeded and every subsequent one
...
//
// The invariant this restores is the one stated at the top of this file: the
// SDK-compacted set stays a subset of the indexer-resolvable set. It must never
