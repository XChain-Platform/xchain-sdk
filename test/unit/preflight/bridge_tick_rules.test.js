...
function assertIndexerConstants() {
...
async function assertWellFormedOptIn() {
...
async function assertActivationNotDeclaredBelowFormatSeven() {
...
async function assertGasTickOffBtcWarning() {
...
function registerReservedNameRules() {
...
function registerTickNamespaceRules() {
    ...
    it('the floor and the vendored roots match the indexer constants', assertIndexerConstants);
}

function registerFormatSevenBaseRules() {
    it('a well-formed opt-in by the owner raises no error and declares the activation and the policy exclusion', assertWellFormedOptIn);
    it('the activation is not declared on formats that exist below it', assertActivationNotDeclaredBelowFormatSeven);
 ...
 */
function registerBridgeChainFieldRules() {
...
function registerRemainingFormatSevenFieldRules() {
...
function registerFormatSevenRules() {
    registerFormatSevenBaseRules();
    registerBridgeChainFieldRules();
    registerRemainingFormatSevenFieldRules();
}

describe('pre-flight bridge landing: ISSUE tick rules', function () {
    describe('reserved names, case-folded and unconditional', registerReservedNameRules);
    describe('tick namespace (activation-keyed, creation-only)', registerTickNamespaceRules);
    describe('format 7: the bridge opt-in', registerFormatSevenRules);
});

function registerDestroyRules() {
    it('the gas tick off BTC is a use-XBRIDGE-v1 warning', assertGasTickOffBtcWarning);
...
describe('pre-flight bridge landing: DESTROY supply-path refusals', registerDestroyRules);
