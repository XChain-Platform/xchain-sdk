...
describe('drift gate sibling resolution (§8.5)', function () {
    ...
    // Three roots, one per outcome the gate must tell apart.
    let absentRoot;      // nothing at the path
    let notIndexerRoot;  // a directory that is not an indexer checkout
    let shapedRoot;      // carries src/actions/, so it RESOLVES and gets compared
    ...
    before(function () {
        ...
        fs.mkdirSync(path.join(notIndexerRoot, 'src', 'handlers'), { recursive: true });
        fs.mkdirSync(path.join(shapedRoot, 'src', 'actions'), { recursive: true });
        this.base = base;
    });

    after(function () {
        if (this.base) fs.rmSync(this.base, { recursive: true, force: true });
    });
    ...
    it('says a directory that is not an indexer checkout is a different fact from an absent one', function () {
        ...
        const absent = runGate({ XCHAIN_INDEXER_PATH: absentRoot });
        const wrong = runGate({ XCHAIN_INDEXER_PATH: notIndexerRoot });
    ...
    it('lets XCHAIN_REQUIRE_SIBLINGS=1 override the declaration', function () {
        ...
        const { code, out } = runGate({
            XCHAIN_INDEXER_PATH: absentRoot,
    ...
    it('still resolves and COMPARES a checkout that carries the handler directory', function () {
        ...
        const { code, out } = runGate({ XCHAIN_INDEXER_PATH: shapedRoot });
